...
const senderEmail = process.env.SENDER_EMAIL || "no-reply@campushub-services.shop"; // Default Resend test email
...
export const sendOrderStatusEmail = async (order) => {
    ...
    const vendorName = order.vendor ? (order.vendor.businessName || order.vendor.fullName || 'CampCart Vendor') : 'CampCart Vendor';
    ...
    try {
        const itemsHtml = (order.items || []).map(item => `
            <tr>
                <td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; color: #4b5563;">${item.product ? item.product.name : 'Product'} <span style="color: #9ca3af;">x${item.quantity}</span></td>
                <td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; text-align: right; color: #111827; font-weight: 500;">₦${(item.price * item.quantity).toLocaleString()}</td>
            </tr>
        `).join('');
                ...
                <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: #f3f4f6; padding: 40px 20px; color: #1f2937;">
                    <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05);">
                        ...
                        <div style="padding: 40px 30px;">
                            ...
                            <p style="font-size: 16px; line-height: 1.6; color: #4b5563;">Hello,</p>
                            <p style="font-size: 16px; line-height: 1.6; color: #4b5563;">Your order status has changed.</p>
                            ...
                            <div style="margin: 30px 0; border: 1px solid #e5e7eb; border-radius: 8px; overflow: hidden;">
                                <div style="background-color: #f9fafb; padding: 15px 20px; border-bottom: 1px solid #e5e7eb;">
                                    <p style="margin: 0; font-size: 14px; color: #4b5563; font-weight: 600; text-transform: uppercase;">Order Details</p>
                                </div>
                                <div style="padding: 20px;">
                                    <p style="margin: 0 0 15px 0; font-size: 15px; color: #4b5563;"><strong>Vendor:</strong> ${vendorName}</p>
                                    <table style="width: 100%; border-collapse: collapse;">
                                        ${itemsHtml}
                                        <tr>
                                            <td style="padding: 12px 0; color: #4b5563;">Delivery Fee</td>
                                            <td style="padding: 12px 0; text-align: right; color: #111827; font-weight: 500;">₦${(order.deliveryCharge || 0).toLocaleString()}</td>
                                        </tr>
                                        <tr>
                                            <td style="padding: 15px 0 0 0; color: #111827; font-weight: 700; font-size: 18px;">Total</td>
                                            <td style="padding: 15px 0 0 0; text-align: right; color: #10b981; font-weight: 700; font-size: 18px;">₦${(order.total || 0).toLocaleString()}</td>
                                        </tr>
                                    </table>
                                </div>
                            </div>
                            ...
                            ${agentDetails ? `
                            <div style="margin: 30px 0; background-color: #f9fafb; border: 1px solid #e5e7eb; padding: 20px; border-radius: 8px;">
                                <h3 style="margin-top: 0; font-size: 16px; color: #111827;">Delivery Agent Details</h3>
                                <p style="margin: 0 0 5px 0; font-size: 15px; color: #4b5563;"><strong>Name:</strong> ${agentDetails.name}</p>
                                ${agentDetails.phone ? `<p style="margin: 0; font-size: 15px; color: #4b5563;"><strong>Phone:</strong> ${agentDetails.phone}</p>` : ''}
                            </div>
                            ` : ''}
...
export const sendNewDeliveryAvailableEmail = async (emails, orderId, vendorName) => {
    if (!resend) {
        console.warn("RESEND_API_KEY not configured. Skipping new delivery email to agents.");
        return;
    }
    ...
    try {
        await resend.emails.send({
            from: senderEmail,
            to: emails,
            subject: `New Delivery Available - #${orderId}`,
            html: `
                <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: #f3f4f6; padding: 40px 20px; color: #1f2937;">
                    <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05);">
                        ...
                        <div style="padding: 40px 30px;">
                            <h2 style="margin-top: 0; color: #111827; font-size: 22px; font-weight: 600;">New Delivery Available!</h2>
                            <p style="font-size: 16px; line-height: 1.6; color: #4b5563;">Hello,</p>
                            <p style="font-size: 16px; line-height: 1.6; color: #4b5563;">A new delivery order is ready for pickup from <strong>${vendorName}</strong>.</p>
                            
                            <div style="margin: 30px 0; background-color: #eff6ff; border-left: 4px solid #3b82f6; padding: 20px; border-radius: 0 8px 8px 0;">
                                <p style="margin: 0 0 8px 0; font-size: 14px; color: #1e3a8a;">Order #${orderId}</p>
                                <p style="margin: 0; font-size: 16px; color: #1d4ed8; font-weight: 600;">Check your agent dashboard to accept this delivery.</p>
                            </div>
                            
                            <p style="font-size: 15px; line-height: 1.6; color: #4b5563;">Log in to the CampCart Agent App to view details and accept the order.</p>
                        </div>
                        <div style="background-color: #f9fafb; padding: 24px; text-align: center; border-top: 1px solid #e5e7eb;">
                            <p style="margin: 0; font-size: 13px; color: #9ca3af;">&copy; ${new Date().getFullYear()} CampCart. All rights reserved.</p>
                        </div>
        ...
        console.log("New delivery available email sent to agents.");
    } catch (error) {
        console.error("Error sending new delivery email:", error);
    }
};
